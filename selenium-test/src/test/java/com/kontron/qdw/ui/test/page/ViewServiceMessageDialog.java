package com.kontron.qdw.ui.test.page;

import net.sourceforge.jbizmo.commons.selenium.junit.*;
import net.sourceforge.jbizmo.commons.annotation.Generated;
import net.sourceforge.jbizmo.commons.selenium.page.imp.primefaces.*;

public class ViewServiceMessageDialog extends AbstractPageObject {
    @Generated
    public static final String RESOURCE_PATH = "/dialog/ViewServiceMessageDialog.jsf";
    @Generated
    public static final String FIELD_ID_CBOSERVICEORDER = "form:tabview1:cboServiceOrder";
    @Generated
    public static final String FIELD_ID_CBOSERIALOBJECT = "form:tabview1:cboSerialObject";
    @Generated
    public static final String FIELD_ID_CBOMATERIALREVISIONMATERIAL = "form:tabview1:cboMaterialRevisionMaterial";
    @Generated
    public static final String FIELD_ID_TXTMATERIALSAPNUMBER = "form:tabview1:txtMaterialSapNumber";
    @Generated
    public static final String FIELD_ID_CBOMATERIALREVISION = "form:tabview1:cboMaterialRevision";
    @Generated
    public static final String FIELD_ID_CBOPLANT = "form:tabview1:cboPlant";
    @Generated
    public static final String FIELD_ID_CBOREPAIRERRORCODE = "form:tabview1:cboRepairErrorCode";
    @Generated
    public static final String FIELD_ID_CBOREPAIRSTATE = "form:tabview1:cboRepairState";
    @Generated
    public static final String FIELD_ID_CBOREPAIRTASK = "form:tabview1:cboRepairTask";
    @Generated
    public static final String FIELD_ID_CBOREPAIRLOCATION = "form:tabview1:cboRepairLocation";
    @Generated
    public static final String FIELD_ID_CBORMATYPE = "form:tabview1:cboRMAType";
    @Generated
    public static final String FIELD_ID_CBOREPAIRSERVICE = "form:tabview1:cboRepairService";
    @Generated
    public static final String FIELD_ID_CBOEXTERNALSUPPLIER = "form:tabview1:cboExternalSupplier";
    @Generated
    public static final String FIELD_ID_TXTORIGIN = "form:tabview1:txtOrigin";
    @Generated
    public static final String FIELD_ID_TXTERRORID = "form:tabview1:txtErrorId";
    @Generated
    public static final String FIELD_ID_TXTFAULTANALYSIS = "form:tabview1:txtFaultAnalysis";
    @Generated
    public static final String FIELD_ID_TXTDEFECTCOMPONENT = "form:tabview1:txtDefectComponent";
    @Generated
    public static final String FIELD_ID_TXTDESIGNATOR = "form:tabview1:txtDesignator";
    @Generated
    public static final String FIELD_ID_TXTANALYSISTEXT = "form:tabview1:txtAnalysisText";
    @Generated
    public static final String FIELD_ID_TXTINTERNALARRIVALDATE = "form:tabview1:txtInternalArrivalDate";
    @Generated
    public static final String FIELD_ID_TXTINTERNALSHIPMENTDATE = "form:tabview1:txtInternalShipmentDate";
    @Generated
    public static final String FIELD_ID_TXTBASICSTARTDATE = "form:tabview1:txtBasicStartDate";
    @Generated
    public static final String FIELD_ID_TXTBASICFINISHDATE = "form:tabview1:txtBasicFinishDate";
    @Generated
    public static final String FIELD_ID_CHKCUSTOMERFAILURE = "form:tabview1:chkCustomerFailure";
    @Generated
    public static final String FIELD_ID_CHKEPIDEMICFAILURE = "form:tabview1:chkEpidemicFailure";
    @Generated
    public static final String FIELD_ID_TXTDELIVERYNOTENUMBER = "form:tabview1:txtDeliveryNoteNumber";
    @Generated
    public static final String FIELD_ID_TXTSERVICEMESSAGEID = "form:tabview1:txtServiceMessageId";
    @Generated
    public static final String FIELD_ID_TXTCAUSETEXT = "form:tabview1:txtCauseText";
    @Generated
    public static final String FIELD_ID_TXTREPAIRDESCRIPTION = "form:tabview1:txtRepairDescription";
    @Generated
    public static final String FIELD_ID_TXTID = "form:tabview1:txtId";
    @Generated
    public static final String FIELD_ID_TXTVERSION = "form:tabview1:txtVersion";
    @Generated
    public static final String FIELD_ID_TXTCREATIONDATE = "form:tabview1:txtCreationDate";
    @Generated
    public static final String FIELD_ID_TXTLASTUPDATE = "form:tabview1:txtLastUpdate";
    @Generated
    public static final String FIELD_ID_TXTINTERNALREPORT = "form:tabview1:txtInternalReport";
    @Generated
    public static final String FIELD_ID_TXTEXTERNALREPORT = "form:tabview1:txtExternalReport";
    @Generated
    public static final String FIELD_ID_TXTCUSTOMERREPORT = "form:tabview1:txtCustomerReport";
    @Generated
    public static final String TAB_PAGE_PANADMINISTRATION = "form:tabview1:tabPanAdministration";
    @Generated
    public static final String TAB_PAGE_PANSERVICEMESSAGE = "form:tabview1:tabPanServiceMessage";
    @Generated
    public static final String TAB_PAGE_PANCOMMENTS = "form:tabview1:tabPanComments";

    /**
     * Constructor
     * @param testContext
     */
    @Generated
    public ViewServiceMessageDialog(SeleniumTestContext testContext) {
        super(testContext);
    }

}