...
public class NavigatorView implements Serializable {
    ...
    public void init() {
        ...
        if (userSession.checkAuthorization(false, ROLE_ADMINISTRATOR, ROLE_READONLY)) {
            ...
            // Serial objects
            if (userSession.checkAuthorization(false, ROLE_ADMINISTRATOR, ROLE_READONLY))
                new DefaultTreeNode<>(VIEW_TYPE, new TreeNavigatorItem(bundle.getString(FORM_SERIALOBJECTVIEW_TITLE),
                        req.getContextPath() + "/view/SerialObjectView.jsf"), itemGroup0004);

            // Serial objects with trace BoM (auf Basis MaterializedArrivalShipment mit TraceBoM)
            if (userSession.checkAuthorization(false, ROLE_ADMINISTRATOR, ROLE_READONLY))
        ...
        if (userSession.checkAuthorization(false, ROLE_ADMINISTRATOR, ROLE_READONLY)) {
            ...
            if (userSession.checkAuthorization(false, ROLE_ADMINISTRATOR)) {
                ...
                // Repair services
                if (userSession.checkAuthorization(false, ROLE_ADMINISTRATOR))
                    new DefaultTreeNode<>(VIEW_TYPE, new TreeNavigatorItem(bundle.getString(FORM_REPAIRSERVICEVIEW_TITLE),
                            req.getContextPath() + "/view/RepairServiceView.jsf"), itemGroup0006);
            ...
            // Open service orders
            if (userSession.checkAuthorization(false, ROLE_ADMINISTRATOR, ROLE_READONLY))
                new DefaultTreeNode<>(VIEW_TYPE, new TreeNavigatorItem(bundle.getString(FORM_SERVICEMESSAGEOPENVIEW_TITLE),
                        req.getContextPath() + "/view/ServiceMessageOpenView.jsf"), itemGroup0005);

            // Service orders (Standard)
            if (userSession.checkAuthorization(false, ROLE_ADMINISTRATOR, ROLE_READONLY, ROLE_SUPERUSER))
            ...
            // X2 Notifications
            if (userSession.checkAuthorization(false, ROLE_ADMINISTRATOR, ROLE_READONLY))
                new DefaultTreeNode<>(VIEW_TYPE, new TreeNavigatorItem(bundle.getString(FORM_X2MESSAGEVIEW_TITLE),
                        req.getContextPath() + "/view/X2MessageView.jsf"), itemGroup0005);
        }
