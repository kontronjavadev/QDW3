...
@Named("traceBoMView")
@ViewScoped
public class TraceBoMView extends SuperView implements Serializable {
    @Generated
    ...
    public void initSearchObject() {
        ...
        new JSFSearchFieldDTO(searchObj, ++colOrderId, TraceBoMSearchDTO.SELECT_SUPPLIERNAME,
                bundle.getString(COL_TRACEBOMVIEW_SUPPLIERNAME), SearchFieldDataTypeEnum.STRING, 200);

        new JSFSearchFieldDTO(searchObj, ++colOrderId, TraceBoMSearchDTO.SELECT_PRODUCTIONDATE,
                bundle.getString(COL_TRACEBOMVIEW_PRODUCTIONDATE), SearchFieldDataTypeEnum.LOCAL_DATE, 80, false);
        ...
        new JSFSearchFieldDTO(searchObj, ++colOrderId, TraceBoMSearchDTO.SELECT_ORDERNUMBER,
                bundle.getString(COL_TRACEBOMVIEW_ORDERNUMBER), SearchFieldDataTypeEnum.STRING, 100);

        new JSFSearchFieldDTO(searchObj, ++colOrderId, TraceBoMSearchDTO.SELECT_LOTNUMBER,
                bundle.getString(COL_TRACEBOMVIEW_LOTNUMBER), SearchFieldDataTypeEnum.STRING, 80);

        new JSFSearchFieldDTO(searchObj, ++colOrderId, TraceBoMSearchDTO.SELECT_CREATIONDATE,
                bundle.getString(LBL_ATTR_ABSTRACTENTITYWITHID_CREATIONDATE), SearchFieldDataTypeEnum.LOCAL_DATE_TIME, 120);

        new JSFSearchFieldDTO(searchObj, ++colOrderId, TraceBoMSearchDTO.SELECT_LASTUPDATE,
    ...
    public void fetchTraceBoMs() {
        ...
        try {
            ...
            if (searchObj.isCount()) {
                if (traceBoMsList.size() == searchObj.getMaxResult()) {
                    countResult = traceBoMService.countAllTraceBoMs(searchObj);
                }
                else {
                    countResult = traceBoMsList.size();
                }
            }
